...
An OS-released lock prevents duplicate simultaneous runners. Every accepted answer is atomically saved under `data/rewrites_sonnet/records/`, with source/config hashes, model identity, request ID, and usage. Restarting skips both the 475 saved Astra answers and already validated Sonnet records. Changed prompts or source records are rejected rather than silently mixed. A snapshot of the exact shared prefix is in `data/rewrites_sonnet/config.json`.
